# Consuming ezyRetire Design System in web apps

Read `artifacts/wealthone-design-system/docs/AGENTS.md` first. This guide covers
...
`artifacts/wealthone-design-system/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/wealthone-design-system/styles.css";
```
...
`node_modules/@workspace/wealthone-design-system/src/components` to `content`.
...
import {
  toast,
  useToast,
} from "@workspace/wealthone-design-system/hooks/use-toast";
```
...
The catalog also exports complete form, overlay, navigation, data-display,
feedback, and layout families under `components/ui/<family>`. Use the living
style guide to review each family's variants and required providers before
composing product UI.

## Verify

After wiring the workspace dependency, import and render
`@workspace/wealthone-design-system/components/ui/button`. Run the app's typecheck
